/**
 * Original ComfyUI workflow for image enhancement
...
export function createWorkflow(imageBase64: string, seed: number): any {
  ...
  const workflow = JSON.parse(JSON.stringify(DEFAULT_COMFYUI_WORKFLOW));

  // Set the dynamic parts
  workflow.input.workflow["55"].inputs.image = imageBase64;
  workflow.input.workflow["26"].inputs.seed = seed;
